...
<body class="template-color-1">
    ...
    <div id="main-wrapper">
        ...
        @if(!empty($rate[$i]->translator_id))
        <form action="{{ url('/rate_finish') }}" method="post">
        @csrf
        <div class="checkout-section section bg_color--5 pb-120 pb-lg-100 pb-md-80 pb-sm-60 pb-xs-50">
            <div class="container faq-wrapper">
                <div class="row">
                    <div class="col-12">
                        <h5>依頼者を評価してください</h5>
                        <input type="hidden" name="jobUrl" value="{{ $rate[$i]->job_url }}">
                        <div>Title : {{ $rate[$i]->job_title }}</div>
                        <div>依頼者 : {{ showUserName($rate[$i]->job_owner_id)}}</div>
                        <div>依頼日時 : {{ showJoborderedDatetime($rate[$i]->job_url)}}</div>
